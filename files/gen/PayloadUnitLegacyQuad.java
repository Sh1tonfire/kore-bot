...
Statusc {
    ...
    public String toString() {
        return "PayloadUnitLegacyQuad#" + this.id;
    }
    ...
    public double sense(LAccess sensor) {
        ...
        switch (sensor) {
            ...
            case payloadCount: {
                ...
                d = payloadUnitLegacyQuad instanceof Payloadc && (pay = payloadUnitLegacyQuad) == payloadUnitLegacyQuad ? pay.payloads().size : 0;
                break;
            }
            default: {
                d = Double.NaN;
    ...
    public void draw() {
        ...
        if (active || this.lastActive != null) {
            ...
            if (tile != null && this.within(plan, Vars.state.rules.infiniteResources ? Float.MAX_VALUE : 220.0f)) {
                ...
                float x1 = PayloadUnitLegacyQuad.vecs[0].x;
                float y1 = PayloadUnitLegacyQuad.vecs[0].y;
                float x2 = close.x;
                float y2 = close.y;
                float x3 = PayloadUnitLegacyQuad.vecs[1].x;
                float y3 = PayloadUnitLegacyQuad.vecs[1].y;
                Draw.z(122.0f);
    ...
    public void update() {
        block83: {
            block84: {
                block86: {
                    block85: {
                        ...
                        if (!this.updateBuilding || !this.canBuild()) break block83;
                        ...
                        core = this.core();
                        if (this.buildPlan() == null) break block83;
                        ...
                        if (!this.within(tile, finalPlaceDst)) break block83;
                        var8_23 = tile.build;
                        if (var8_23 instanceof ConstructBlock.ConstructBuild && (cb = (ConstructBlock.ConstructBuild)var8_23) == (ConstructBlock.ConstructBuild)var8_23) break block84;
                        if (current.initialized || current.breaking || !Build.validPlace(current.block, this.team, current.x, current.y, current.rotation)) break block85;
                        v1 = hasAll = infinite != false || current.isRotation(this.team) != false || Structs.contains(current.block.requirements, (Boolf<ItemStack>)LambdaMetafactory.metafactory(null, null, null, (Ljava/lang/Object;)Z, lambda$update$12(mindustry.gen.Building mindustry.type.ItemStack ), (Lmindustry/type/ItemStack;)Z)((Building)core)) == false;
                        if (hasAll) {
                    ...
                    if (current.initialized || !current.breaking || !Build.validBreak(this.team, current.x, current.y)) break block86;
...

            {
                if (tile.build instanceof ConstructBlock.ConstructBuild && !current.initialized) {
                    Core.app.post((Runnable)LambdaMetafactory.metafactory(null, null, null, ()V, lambda$update$13(mindustry.world.Tile mindustry.entities.units.BuildPlan ), ()V)((PayloadUnitLegacyQuad)this, (Tile)tile, (BuildPlan)current));
                    current.initialized = true;
                }
                if ((core != null || infinite) && (hasAll = tile.build) instanceof ConstructBlock.ConstructBuild && (entity = (ConstructBlock.ConstructBuild)hasAll) == (ConstructBlock.ConstructBuild)hasAll) {
                    if (current.breaking) {
        ...
        for (WeaponMount mount : this.mounts) {
            ...
            v2 = shootAngle = weapon.rotate != false ? weaponRotation + 90.0f : Angles.angle(shootX, shootY, mount.aimX, mount.aimY) + (this.rotation - this.angleTo(mount.aimX, mount.aimY));
            ...
            if (weapon.otherSide != -1 && weapon.alternate && mount.side == weapon.flipSprite && mount.reload + Time.delta * this.reloadMultiplier > weapon.reload / 2.0f && mount.reload <= weapon.reload / 2.0f) {
                ...
                v3 = mount.side = mount.side == false;
            }
        ...
        if (this.formation != null) {
            ...
            this.controlling.removeAll((Boolf<Unit>)LambdaMetafactory.metafactory(null, null, null, (Ljava/lang/Object;)Z, lambda$update$14(mindustry.gen.Unit ), (Lmindustry/gen/Unit;)Z)((PayloadUnitLegacyQuad)this));
        }
        floor = this.floorOn();
    ...
    private /* synthetic */ boolean lambda$update$14(Unit u) {
        FormationAI ai;
