...
Statusc {
    ...
    public transient Color trailColor;
    ...
    public double sense(LAccess sensor) {
        ...
        switch (sensor) {
            ...
            case payloadCount: {
                ...
                d = unitWaterMove instanceof Payloadc && (pay = (Payloadc)((Object)unitWaterMove)) == (Payloadc)((Object)unitWaterMove) ? pay.payloads().size : 0;
                break;
            }
            default: {
                d = Double.NaN;
    ...
    public void draw() {
        ...
        if (active || this.lastActive != null) {
            ...
            if (tile != null && this.within(plan, Vars.state.rules.infiniteResources ? Float.MAX_VALUE : 220.0f)) {
                ...
                float x1 = UnitWaterMove.vecs[0].x;
                float y1 = UnitWaterMove.vecs[0].y;
                float x2 = close.x;
                float y2 = close.y;
                float x3 = UnitWaterMove.vecs[1].x;
                float y3 = UnitWaterMove.vecs[1].y;
                Draw.z(122.0f);
    ...
    public void update() {
        block84: {
            block85: {
                block87: {
                    block86: {
                        ...
                        if (!this.updateBuilding || !this.canBuild()) break block84;
                        ...
                        core = this.core();
                        if (this.buildPlan() == null) break block84;
                        ...
                        if (!this.within(tile, finalPlaceDst)) break block84;
                        var8_25 = tile.build;
                        if (var8_25 instanceof ConstructBlock.ConstructBuild && (cb = (ConstructBlock.ConstructBuild)var8_25) == (ConstructBlock.ConstructBuild)var8_25) break block85;
                        if (current.initialized || current.breaking || !Build.validPlace(current.block, this.team, current.x, current.y, current.rotation)) break block86;
                    ...
                    if (current.initialized || !current.breaking || !Build.validBreak(this.team, current.x, current.y)) break block87;
...

            {
                if (tile.build instanceof ConstructBlock.ConstructBuild && !current.initialized) {
                    Core.app.post((Runnable)LambdaMetafactory.metafactory(null, null, null, ()V, lambda$update$13(mindustry.world.Tile mindustry.entities.units.BuildPlan ), ()V)((UnitWaterMove)this, (Tile)tile, (BuildPlan)current));
        ...
        for (i = 0; i < 2; ++i) {
            t = i == 0 ? this.tleft : this.tright;
            t.length = this.type.trailLength;
            sign = i == 0 ? -1 : 1;
            cx = Angles.trnsx(this.rotation - 90.0f, this.type.trailX * (float)sign, this.type.trailY) + this.x;
            cy = Angles.trnsy(this.rotation - 90.0f, this.type.trailX * (float)sign, this.type.trailY) + this.y;
            t.update(cx, cy);
        }
        ...
        if (this.formation != null) {
            ...
            this.controlling.removeAll((Boolf<Unit>)LambdaMetafactory.metafactory(null, null, null, (Ljava/lang/Object;)Z, lambda$update$14(mindustry.gen.Unit ), (Lmindustry/gen/Unit;)Z)((UnitWaterMove)this));
        }
        floor = this.floorOn();
