...
Statusc {
    ...
    public String toString() {
        return "LegsUnitLegacyToxopid#" + this.id;
    }
    ...
    public double sense(LAccess sensor) {
        ...
        switch (sensor) {
            ...
            case payloadCount: {
                Payloadc pay;
                LegsUnitLegacyToxopid legsUnitLegacyToxopid = this;
                d = legsUnitLegacyToxopid instanceof Payloadc && (pay = (Payloadc)((Object)legsUnitLegacyToxopid)) == (Payloadc)((Object)legsUnitLegacyToxopid) ? pay.payloads().size : 0;
                break;
            }
            default: {
                d = Double.NaN;
        ...
        block94: {
            block95: {
                block97: {
                    block96: {
                        ...
                        trns = this.moveSpace * 0.85f * this.type.legTrns;
                        moveOffset = Tmp.v4.trns(rot, trns);
                        moving = this.moving();
                        for (i = 0; i < this.legs.length; ++i) {
                            dstRot = this.legAngle(rot, i);
                            baseOffset = Tmp.v5.trns(dstRot, this.type.legBaseOffset).add(this.x, this.y);
                            l = this.legs[i];
                            ...
                            group = stage % div;
                            move = i % div == group;
                            side = i < this.legs.length / 2;
                            v1 = backLeg = Math.abs((float)i + 0.5f - (float)this.legs.length / 2.0f) <= 0.501f;
                            if (backLeg && this.type.flipBackLegs) {
                                side = side == false;
                            }
                            l.moving = move;
                            v2 = l.stage = moving != false ? stageF % 1.0f : Mathf.lerpDelta(l.stage, 0.0f, 0.1f);
                            if (l.group != group) {
                            ...
                            legDest = Tmp.v1.trns(dstRot, legLength * this.type.legLengthScl).add(baseOffset).add(moveOffset);
                            jointDest = Tmp.v2;
                        ...
                        for (WeaponMount mount : this.mounts) {
                            ...
                            v3 = shootAngle = weapon.rotate != false ? weaponRotation + 90.0f : Angles.angle(shootX, shootY, mount.aimX, mount.aimY) + (this.rotation - this.angleTo(mount.aimX, mount.aimY));
                            ...
                            if (weapon.otherSide != -1 && weapon.alternate && mount.side == weapon.flipSprite && mount.reload + Time.delta * this.reloadMultiplier > weapon.reload / 2.0f && mount.reload <= weapon.reload / 2.0f) {
                                ...
                                v4 = mount.side = mount.side == false;
                            }
                        ...
                        if (this.formation != null) {
                            ...
                            this.controlling.removeAll((Boolf<Unit>)LambdaMetafactory.metafactory(null, null, null, (Ljava/lang/Object;)Z, lambda$update$11(mindustry.gen.Unit ), (Lmindustry/gen/Unit;)Z)((LegsUnitLegacyToxopid)this));
                        }
                        this.stack.amount = Mathf.clamp(this.stack.amount, 0, this.itemCapacity());
                        ...
                        if (!this.updateBuilding || !this.canBuild()) break block94;
                        ...
                        core = this.core();
                        if (this.buildPlan() == null) break block94;
                        ...
                        if (!this.within(tile, finalPlaceDst)) break block94;
                        mountX = tile.build;
                        if (mountX instanceof ConstructBlock.ConstructBuild && (cb = (ConstructBlock.ConstructBuild)mountX) == (ConstructBlock.ConstructBuild)mountX) break block95;
                        if (current.initialized || current.breaking || !Build.validPlace(current.block, this.team, current.x, current.y, current.rotation)) break block96;
                        v5 = hasAll = infinite != false || current.isRotation(this.team) != false || Structs.contains(current.block.requirements, (Boolf<ItemStack>)LambdaMetafactory.metafactory(null, null, null, (Ljava/lang/Object;)Z, lambda$update$12(mindustry.gen.Building mindustry.type.ItemStack ), (Lmindustry/type/ItemStack;)Z)((Building)core)) == false;
                        if (hasAll) {
                    ...
                    if (current.initialized || !current.breaking || !Build.validBreak(this.team, current.x, current.y)) break block97;
...

            {
                if (tile.build instanceof ConstructBlock.ConstructBuild && !current.initialized) {
                    Core.app.post((Runnable)LambdaMetafactory.metafactory(null, null, null, ()V, lambda$update$13(mindustry.world.Tile mindustry.entities.units.BuildPlan ), ()V)((LegsUnitLegacyToxopid)this, (Tile)tile, (BuildPlan)current));
                    current.initialized = true;
                }
                if ((core != null || infinite) && (var8_40 = tile.build) instanceof ConstructBlock.ConstructBuild && (entity = (ConstructBlock.ConstructBuild)var8_40) == (ConstructBlock.ConstructBuild)var8_40) {
                    if (current.breaking) {
    ...
    public void draw() {
        ...
        if (active || this.lastActive != null) {
            ...
            if (tile != null && this.within(plan, Vars.state.rules.infiniteResources ? Float.MAX_VALUE : 220.0f)) {
                ...
                float x1 = LegsUnitLegacyToxopid.vecs[0].x;
                float y1 = LegsUnitLegacyToxopid.vecs[0].y;
                float x2 = close.x;
                float y2 = close.y;
                float x3 = LegsUnitLegacyToxopid.vecs[1].x;
                float y3 = LegsUnitLegacyToxopid.vecs[1].y;
                Draw.z(122.0f);
    ...
    private /* synthetic */ void lambda$update$13(Tile tile, BuildPlan current) {
        Events.fire(new EventType.BuildSelectEvent(tile, this.team, this, current.breaking));
    }

    private static /* synthetic */ boolean lambda$update$12(Building core, ItemStack i) {
        return core != null && !core.items.has(i.item);
    }

    private /* synthetic */ boolean lambda$update$11(Unit u) {
        FormationAI ai;
